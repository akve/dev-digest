...
Non-obvious findings from working in the pure review engine. Append-only — never overwrite, only add. Each entry: date + what + why non-obvious + `file:line` evidence.
...
<!-- Dependency quirks, LLM SDK / prompt pipeline gotchas, config surprises -->
